  ...
  struct FractionSeparation
  {
    ...
    std::string fractionNames[4][12] = { 
                                  { "FractionReco", "WFractionReco", "PtFractionReco","WPtFractionReco",
                                    ...
                                    "WHarmPtAvgFractionReco", "HarmWPtAvgFractionReco"}, 
                                  { "FractionRecoMatched", "WFractionRecoMatched", "PtFractionRecoMatched",
                                    "WPtFractionRecoMatched", "Pt2FractionRecoMatched",  
                                    "WPt2FractionRecoMatched", "HarmPtFractionRecoMatched", 
                                    "WHarmPtFractionRecoMatched", "HarmWPtFractionRecoMatched",
                                    "HarmPtAvgFractionRecoMatched", "WHarmPtAvgFractionRecoMatched", 
                                    "HarmWPtAvgFractionRecoMatched"},
                                  { "FractionSim", "WFractionSim", "PtFractionSim", "PtFractionSim", "Pt2FractionSim", 
                                    "Pt2FractionSim"}, 
    ...
    : verbose_(iConfig.getUntrackedParameter<bool>("verbose", false)),
      ...
      root_folder_(
          iConfig.getUntrackedParameter<std::string>("root_folder",
                                                "Validation/Vertices")),
      doDetailedHistograms_(iConfig.getUntrackedParameter<bool>("doDetailedHistograms")),
...
void PrimaryVertexAnalyzer4PUSlimmed::bookHistograms(
  ...
  for (auto const& l : reco_vertex_collections_) {
    ...
    auto book1d = [&](const std::string& name, int bins, double min, double max) {
    ...
    // Shared tracks
    std::string numeratorNames[4] = {"RecoAllAssoc2GenSingleMatched", "RecoAllAssoc2GenMultiMatched",
                                  "GenAllAssoc2RecoSingleMatched", "GenAllAssoc2RecoMultiMatched"};
    std::string numeratorNames2[4] = {"ReconstructedAllAssoc2GenSingleMatched",                                   
                                  "ReconstructedAllAssoc2GenMultiMatched",
                                  "GeneratedAllAssoc2RecoSingleMatched", 
                                  "GeneratedAllAssoc2RecoMultiMatched"};
    std::string denominatorNames[4] = {"Reco", "RecoMatched", "Sim", "SimMatched"};

    for(const std::string& numerator: numeratorNames)
    {
      for(const std::string& denominator: denominatorNames)
        book1d(std::string(numerator+"_SharedTrackFraction"+denominator), 50, 0, 1);
    }

    if(doDetailedHistograms_)
    {
      std::string associationNames[12] = {"", "W", "Pt", "WPt", "Pt2", "WPt2", "HarmPt", "WHarmPt","HarmWPt",
                                        "HarmPtAvg", "WHarmPtAvg", "HarmWPtAvg" };

      for(const std::string& numerator: numeratorNames)
      {
        for(const std::string& denominator: denominatorNames)
        {
          for(const std::string& association: associationNames)
          {
            // separated plots
            for(int indexL = 0; indexL < 4; ++indexL)
            {
              // book only Pt- and Pt2- plots for Sim and SimMatched plots for GenAllAssoc2Reco- plots
              if( numerator.compare(0,3,"Gen") && (denominator.compare("Sim") == 0 ||
                  denominator.compare("SimMatched") == 0) )
              {
                if( !( association.compare("Pt") || association.compare("Pt2") ) )
                  continue;
              }

              if(indexL == 0)
                book1d(std::string(numerator+"_SharedTrack"+association+"Fraction"+denominator), 50, 0, 1);
              //separated plots
              else
                book1d(std::string(numerator+"_SharedTrack"+association+"Fraction"+denominator+"_separated"+std::to_string(indexL)), 50, 0, 1);
            }
          }
        }
      }
...
void PrimaryVertexAnalyzer4PUSlimmed::fillRecoAssociatedGenVertexHistograms(
  ...
  for(size_t i=0; i<v.rec_vertices.size(); ++i) {
    ...
    vertexAssociation::TrackFraction tfRecoMatched = calculateVertexSharedTrackFractions(*(v.sim_vertex.get()), *(v.rec_vertices[i]), *s2r_, *r2s_, vertexAssociation::DividerType::RecoMatched);
    vertexAssociation::TrackFraction tfSim = calculateVertexSharedTrackFractions(*(v.sim_vertex.get()), *(v.rec_vertices[i]), *s2r_, *r2s_, vertexAssociation::DividerType::Sim);
...
void PrimaryVertexAnalyzer4PUSlimmed::fillGenAssociatedRecoVertexHistograms(
  ...
  for(size_t i=0; i<v.sim_vertices.size(); ++i) {
    ...
    vertexAssociation::TrackFraction tfReco = calculateVertexSharedTrackFractions(*(v.recVtx), *(v.sim_vertices[i]), *s2r_, *r2s_, vertexAssociation::DividerType::Reco);
    vertexAssociation::TrackFraction tfRecoMatched = calculateVertexSharedTrackFractions(*(v.recVtx), *(v.sim_vertices[i]), *s2r_, *r2s_, vertexAssociation::DividerType::RecoMatched);
    vertexAssociation::TrackFraction tfSim = calculateVertexSharedTrackFractions(*(v.recVtx), *(v.sim_vertices[i]), *s2r_, *r2s_, vertexAssociation::DividerType::Sim);
